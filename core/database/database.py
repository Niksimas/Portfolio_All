...
def save_new_user(user_id: int, link: str) -> None:
    with sqlite3.connect(f"{home}/database/main_data.db") as connect:
        ...
        if bool(cursor.fetchall()[0][0]):
            return
        set_statistic("new_user", user_id)
...
def get_project_data(project_id: int) -> dict:
    try:
        with sqlite3.connect(f"{home}/database/main_data.db") as connect:
            cursor = connect.cursor()
            cursor.execute(f'SELECT * FROM main.project WHERE id=$1', [project_id])
            data = cursor.fetchall()[0]
            result = {"id": data[0], "name_project": data[1], "description": data[2], "name_photo": data[3]}
            return result
    except IndexError:
        return {}
...
def get_project_all_id() -> list:
    try:
        with sqlite3.connect(f"{home}/database/main_data.db") as connect:
            cursor = connect.cursor()
            cursor.execute(f'SELECT id FROM main.project ')
...
def save_new_project(data: dict) -> None:
    with sqlite3.connect(f"{home}/database/main_data.db") as connect:
        cursor = connect.cursor()
        cursor.execute('INSERT INTO main.project (name, description, name_photo) VALUES(?, ?, ?);',
                       [data["name"], data["description"], data["name_photo"]])
